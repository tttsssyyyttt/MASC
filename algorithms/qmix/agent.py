...
class QMIXAgent(MADRLAgent):
    ...
    def __init__(
        ...
        self.n_action_branches = [1 if k == 0 else k for k in self.n_upstream]
            ...
            for i in range(self.n):
                ...
                if np.random.random() < epsilon:
                    if k == 0:
                        ...
                        action_i = [
                            int(np.random.randint(0, self.max_order + 1))
                            for _ in range(k)
                        ]
                else:
                    with torch.no_grad():
    ...
    def _branch_action_value(self, action, branch_idx: int, n_upstream: int) -> int:
        ...
        # New QMIX action: one branch per upstream, action is a list.
        if isinstance(action, (list, tuple, np.ndarray)):
            if branch_idx < len(action):
                return int(action[branch_idx])
            return 0

        # Backward compatibility for old q + alpha action.
        if isinstance(action, dict):
            if branch_idx == 0:
                return int(action.get("q", 0))
            alpha = action.get("alpha", [])
